...
## Windows version
docker run --network mynetwork --rm confluentinc/cp-kafka:7.7.1 kafka-topics --bootstrap-server kafka-1:29092,kafka-2:39092,kafka-3:49092 --create --topic mytopic --partitions 3 --replication-factor 3
...
#base64 /dev/urandom | head -c 10000 | egrep -ao "\w" | tr -d '\n' > random.txt  && kafka-producer-perf-test --topic mytopic --num-records 10000 --throughput 5 --payload-file random.txt --producer-props acks=1 bootstrap.servers=kafka-1:29092,kafka-2:39092,kafka-3:49092 --payload-delimiter A
...
## Windows version
docker run --rm --network mynetwork confluentinc/cp-kafka:7.7.1 kafka-console-consumer --bootstrap-server kafka-1:29092,kafka-2:39092,kafka-3:49092 --topic mytopic
