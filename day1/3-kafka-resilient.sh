...
## Windows version
docker run --network mynetwork --rm confluentinc/cp-kafka:7.4.6 kafka-topics --bootstrap-server kafka-1:29092,kafka-2:39092,kafka-3:49092 --create --topic mytopic --partitions 3 --replication-factor 3
...
## Windows version
docker run  -it  --network mynetwork  --rm  confluentinc/cp-kafka:7.4.6 /bin/bash
...
## Windows version
docker run --rm --network mynetwork confluentinc/cp-kafka:7.4.6 kafka-console-consumer --bootstrap-server kafka-1:29092,kafka-2:39092,kafka-3:49092 --topic mytopic
