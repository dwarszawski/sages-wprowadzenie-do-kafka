...
public class TimeWindowOperatorTest {
  ...
  private static void produceInputs(final String bootstrapServers, final String schemaRegistryUrl) throws IOException {
    final String[] users = {"adam", "michal", "ewa", "damian", "maria", "barbara", "janusz", "lukasz", "jan"};
    ...
    for (final String user : users) {
      ...
      IntStream.range(0, random.nextInt(10))
          ...
          record -> producer.send(new ProducerRecord<>(TimeWindowOperator.PAGE_VIEWS, null, record))
      );
    }
    producer.flush();
  }
  ...
  private static void consumeOutput(final String bootstrapServers, final String schemaRegistryUrl) {
    ...
    consumer.subscribe(Collections.singleton(TimeWindowOperator.TOP_INSDUSTRY_NEWS));
    while (true) {
